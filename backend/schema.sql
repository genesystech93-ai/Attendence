...
CREATE TABLE public.offices (
    ...
    auto_checkout_time TIME NOT NULL DEFAULT '20:00:00',
...
-- 9. Auto Checkout Function
-- This function finds any attendance logs from previous days that are missing a check_out
-- and sets the check_out timestamp to the exact auto_checkout_time defined in their office config.
CREATE OR REPLACE FUNCTION public.run_auto_checkout()
RETURNS VOID AS $$
BEGIN
    UPDATE public.attendance_logs al
    SET 
        check_out = (date_trunc('day', al.check_in) + o.auto_checkout_time)::TIMESTAMPTZ,
        status = 'present' -- or maybe 'forgot_checkout' if we want to track it
    FROM public.offices o
    JOIN public.users u ON u.office_id = o.id
    WHERE al.user_id = u.id
      AND al.check_out IS NULL
      AND date_trunc('day', al.check_in) < date_trunc('day', now());
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
